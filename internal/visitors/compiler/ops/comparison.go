...
func (o *Op) CompNumbers(equal bool, ra string, rb string, rx string) string {
	cmd := ""
	// If the numbers are equal, the result is 1, otherwise 0
	cmd += o.MoveConst("0", rx)
	cmd += o.MoveScore(ra, ra)
	cmd += o.MoveScore(rb, rb)
	cmd += o.ExecCond(fmt.Sprintf("score %s %s = %s %s", ra, o.Namespace, rb, o.Namespace), equal, o.MoveConst("1", rx))
	return cmd
}
