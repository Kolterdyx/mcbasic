...
import (
	...
	"github.com/Kolterdyx/mcbasic/internal/compiler/mapping/command_mapper"
...
type Compiler struct {
	...
	scopes map[string][]TypedIdentifier

	commandMapper mapping.Mapper
...
func NewCompiler(
	...
	c.scopes = make(map[string][]TypedIdentifier)
...
func (c *Compiler) Compile(program parser.Program) {
	...
	c.commandMapper = command_mapper.NewCommandMapper(
		c.Namespace,
		program.Structs,
	)
	...
	structDefFunctionSource += c.commandMapper.Return()
...
func (c *Compiler) createFunction(fullName string, source string, args []interfaces.FuncArg, returnType types.ValueType) {
	...
	err := os.WriteFile(c.getFuncPath(namespace)+"/"+filename, []byte(c.commandMapper.MacroLineIndicator(source)), 0644)
	if err != nil {
		log.Fatalln(err)
	}
}
...
func (c *Compiler) Compare(expr expressions.BinaryExpr, ra string, rb string, rx string) string {
	...
	case tokens.EqualEqual:
		if expr.Left.ReturnType() != expr.Right.ReturnType() {
			// Return false
			cmd += c.commandMapper.MakeConst(nbt.NewInt(0), rx)
		} else {
			if expr.Left.ReturnType() == types.IntType {
				cmd += c.commandMapper.EqNumbers(ra, rb, rx)
			} else if expr.Left.ReturnType() == types.StringType {
				cmd += c.commandMapper.EqStrings(ra, rb, rx)
			}
		}
	case tokens.BangEqual:
		if expr.Left.ReturnType() != expr.Right.ReturnType() {
			// Return true
			cmd += c.commandMapper.MakeConst(nbt.NewInt(1), rx)
		} else {
			if expr.Left.ReturnType() == types.IntType {
				cmd += c.commandMapper.NeqNumbers(ra, rb, rx)
			} else if expr.Left.ReturnType() == types.StringType {
				cmd += c.commandMapper.NeqStrings(ra, rb, rx)
			}
	...
	case tokens.Greater:
		...
		cmd += c.commandMapper.GtNumbers(ra, rb, rx)
	case tokens.GreaterEqual:
		if expr.Left.ReturnType() != types.IntType {
			c.error(expr.SourceLocation, fmt.Sprintf("Invalid type in binary operation: %s", expr.Left.ReturnType().ToString()))
		}
		cmd += c.commandMapper.GteNumbers(ra, rb, rx)
	case tokens.Less:
		if expr.Left.ReturnType() != types.IntType {
			c.error(expr.SourceLocation, fmt.Sprintf("Invalid type in binary operation: %s", expr.Left.ReturnType().ToString()))
		}
		cmd += c.commandMapper.LtNumbers(ra, rb, rx)
	case tokens.LessEqual:
		if expr.Left.ReturnType() != types.IntType {
			c.error(expr.SourceLocation, fmt.Sprintf("Invalid type in binary operation: %s", expr.Left.ReturnType().ToString()))
		}
		cmd += c.commandMapper.LteNumbers(ra, rb, rx)
	default:
