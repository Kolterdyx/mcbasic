...
func (c *Compiler) VisitFunctionDeclaration(stmt ast.FunctionDeclarationStmt) any {
	...
	if !stmt.Autogenerated {
		...
		funcFile := path.Join(paths.FunctionBranches, fmt.Sprintf("%s_%s", utils.FileSpecifier(stmt.GetSourceLocation().File), funcName))
		wrapperFuncFile := fmt.Sprintf("%s_%s", utils.FileSpecifier(stmt.GetSourceLocation().File), funcName)
...
func (c *Compiler) VisitIf(stmt ast.IfStmt) any {
	...
	c.branchCounter++
	thenBranchName := path.Join(paths.FunctionBranches, fmt.Sprintf("%s_%s_%d_if", utils.FileSpecifier(c.currentScope.ScopeName()), c.currentScope.ScopeName(), c.branchCounter))
	elseBranchName := path.Join(paths.FunctionBranches, fmt.Sprintf("%s_%s_%d_else", utils.FileSpecifier(c.currentScope.ScopeName()), c.currentScope.ScopeName(), c.branchCounter))
