...
func (i Instruction) ToMCCommand() string {
	...
	case If:
		return fmt.Sprintf("execute if score %s %s matches 1.. run %s\n", i.Args[0], i.DPNamespace, parseInstruction(i.Args[1:], i.DPNamespace, i.Storage).ToMCCommand())
	case Unless:
		return fmt.Sprintf("execute unless score %s %s matches 1.. run %s\n", i.Args[0], i.DPNamespace, parseInstruction(i.Args[1:], i.DPNamespace, i.Storage).ToMCCommand())
	case Ret:
		return "return 0\n"
	case Call, Branch:
